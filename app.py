...
if month in [11, 12]:
    st.info("Выбранный месяц приходится на праздничный сезон. Исторические данные о продажах могут отличаться в течение этого периода.")
elif month in [1, 2]:
    st.info("Выбранный месяц относится к периоду начала года. Исторические показатели продаж могут отличаться от показателей более поздних месяцев")
